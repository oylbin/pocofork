...
class PocoConan(ConanFile):
    ...
    def configure(self):
        ...
        if Version(self.version) >= "1.12.0":
            foundation_external_dependencies = self._poco_component_tree[
                "Foundation"].external_dependencies
            self._poco_component_tree["Foundation"] = self._poco_component_tree["Foundation"]._replace(
                external_dependencies=list(map(lambda x: 'pcre2::pcre2' if x == 'pcre::pcre' else x, foundation_external_dependencies)))
    ...
    def generate(self):
        ...
        for comp in self._poco_component_tree.values():
            if comp.option:
                tc.variables[comp.option.upper()] = self.options.get_safe(
                    comp.option, False)
        tc.variables["POCO_UNBUNDLED"] = False
        if self.settings.os == "Windows":
        ...
        if self.options.get_safe("enable_data_postgresql", False):
            ...
            tc.variables["PostgreSQL_ROOT_LIBRARY_DIRS"] = ";".join(
                self._dep_lib_paths("libpq"))
        if self.options.get_safe("enable_data_mysql", False):
            ...
            tc.variables["MYSQL_INCLUDE_DIR"] = ";".join(
                self._dep_include_paths("libmysqlclient"))
        ...
        if self.options.enable_apacheconnector:
            tc.variables["APR_ROOT_DIR"] = self.dependencies["apr"].package_folder.replace(
                "\\", "/")
            tc.variables["APR_ROOT_INCLUDE_DIRS"] = ";".join(
                self._dep_include_paths("apr"))
            tc.variables["APR_ROOT_LIBRARY_DIRS"] = ";".join(
                self._dep_lib_paths("apr"))
            ...
            tc.variables["APRUTIL_ROOT_LIBRARY_DIRS"] = ";".join(
                self._dep_lib_paths("apr-util"))
        # Disable fork
    ...
    #             "cmake_file_name") or "libmysqlclient"
    #         replace_in_file(
    #             self,
    ...
    #         )
    #         mysql_target_name = mysql_cpp_info.get_property(
    #             "cmake_target_name") or "libmysqlclient::libmysqlclient"
    #         replace_in_file(
    #             self,
    #             os.path.join(self.source_folder, "Data",
    ...
    def package_info(self):
        ...
        for compname, comp in self._poco_component_tree.items():
            if comp.option is None or self.options.get_safe(comp.option):
                ...
                requires = [
                    f"poco_{dependency.lower()}" for dependency in comp.dependencies] + comp.external_dependencies
                self.cpp_info.components[conan_component].set_property(
                    "cmake_target_name", f"Poco::{compname}")
                self.cpp_info.components[conan_component].set_property(
                    "cmake_file_name", compname)
                ...
                if comp.is_lib:
                    self.cpp_info.components[conan_component].libs = [
                        f"Poco{compname}{suffix}"]
        ...
        if self.options.log_debug:
            self.cpp_info.components["poco_foundation"].defines.append(
                "POCO_LOG_DEBUG")
        ...
        if self.options.enable_data_odbc:
            if self.settings.os == "Windows":
                self.cpp_info.components["poco_dataodbc"].system_libs.extend(
                    ["odbc32", "odbccp32"])
            else:
                self.cpp_info.components["poco_dataodbc"].requires.append(
                    "odbc::odbc")
        self.cpp_info.components["poco_foundation"].defines.append(
            "POCO_UNBUNDLED")
